...
mod tests {
    ...
    fn simple_test() {
        ...
        tokio::runtime::Builder::new_current_thread()
            ...
            .unwrap()
            .block_on(async {
                let limiter = RateLimiter::new(1, std::time::Duration::from_secs(CHARGE_EVERY));
                ...
                let another_handle = {
                    ...
                    tokio::spawn(async move {
                        ...
                        limiter.wait().await;
                        // some computation
                        assert_eq!(start.elapsed().as_secs(), CHARGE_EVERY);
                    })
                };

                let another_another_handle = {
                    let limiter = limiter.clone();
                    tokio::spawn(async move {
                        let start = std::time::Instant::now();
                        limiter.wait().await;
                        // some computation
                        assert_eq!(start.elapsed().as_secs(), CHARGE_EVERY * 2);
                    })
                };

                handle.await.unwrap();
                another_handle.await.unwrap();
                another_another_handle.await.unwrap();
            });
    }
}
